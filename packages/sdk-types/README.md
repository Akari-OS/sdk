...
TypeScript type definitions for the **AKARI App SDK**.

App developers `import type { ... }` from this package to get full
IDE-level type checking and hover documentation when building AKARI Apps.
...
import type { AppAPI }        from "@akari-os/sdk/inter-app"
...
| [`/inter-app`](./src/inter-app.ts) | Inter-App API — `AppAPI`, `HandoffPayload`, `HandoffHandler` |
| [`/permission`](./src/permission.ts) | Permission API — scoped gates, HITL, OAuth, audit records |
| [`/skill`](./src/skill.ts) | Skill API — typed callable units shared between Apps |
...
// Declare an App-supplied agent spec
