 ...
 *
 * Parses and validates the App Manifest (`akari.toml`) using @iarna/toml.
...
/**
 * Validate a parsed TOML object as an AppManifest.
...
export function validateManifest(raw: unknown): ManifestValidationResult {
  ...
  if (!data.app || typeof data.app !== "object") {
    errors.push({ field: "[app]", message: "Missing required [app] section", code: "MISSING_APP_SECTION" });
  } else {
    const app = data.app as Record<string, unknown>;

    if (!app.id || typeof app.id !== "string") {
      errors.push({ field: "[app] id", message: "Missing required field: id", code: "MISSING_FIELD" });
    }
    if (!app.name || typeof app.name !== "string") {
      errors.push({ field: "[app] name", message: "Missing required field: name", code: "MISSING_FIELD" });
    }
    if (!app.tier || typeof app.tier !== "string") {
      errors.push({ field: "[app] tier", message: "Missing required field: tier", code: "MISSING_FIELD" });
    } else if (app.tier !== "full" && app.tier !== "mcp-declarative") {
      errors.push({
        field: "[app] tier",
        message: `Invalid tier "${app.tier}". Must be "full" or "mcp-declarative"`,
        code: "INVALID_TIER",
      });
    }
    if (app.sdk && typeof app.sdk !== "string") {
      errors.push({ field: "[app] sdk", message: "Field sdk must be a string (semver range, e.g. '>=0.1.0 <1.0')", code: "INVALID_SDK_RANGE" });
    }
    if (!app.category) {
      warnings.push("[app] category is missing — recommended for Marketplace categorisation");
    }
  }
  ...
  const tier = (data.app as Record<string, unknown>)?.tier as string | undefined;
  ...
  return {
    ...
    errors: [],
    warnings,
    manifest: raw as unknown as AppManifest,
  };
}
...
export async function loadAndValidateManifest(
  ...
  const manifestPath = path.join(appDir, "akari.toml");
