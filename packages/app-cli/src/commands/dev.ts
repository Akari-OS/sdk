 ...
 *
 * Local development server: starts the MCP server for the app in the current
 ...
 *   - For Full Tier apps, optionally serve the React Panel bundle (Vite dev server)
 *
 * TODO (Phase 3 — Shell mount):
 *   - akari dev should be able to hot-mount the app into a running local Shell
...
export async function runDev(appDir: string, port: number): Promise<void> {
