 ...
 *
 * Scaffolds a new AKARI app directory from Handlebars templates.
 * Spec contract: AKARI-HUB-024 §6.9 (Toolchain) + §6.2 (App Tier).
...
interface TemplateVars {
  appId: string;          // e.g. "com.user.my-app"
  appName: string;        // e.g. "My App"
  appSlug: string;        // e.g. "my-app"
...
/** Convert the last segment of a reverse-domain ID to snake_case app-short-id (ADR-011) */
...
export function registerCreateCommand(program: Command): void {
  program
    .command("create <name>")
    .description(
      "Scaffold a new AKARI app (Full or MCP-Declarative tier)"
    )
    .option(
      "--tier <tier>",
      "App tier: full | mcp-declarative (interactive if omitted)"
    )
    .option(
      "--author <author>",
    ...
    .action(async (name: string, opts: { tier?: string; author?: string; category?: string }) => {
      ...
      if (!/^[a-z][a-z0-9-]*$/.test(name)) {
        console.error(
          chalk.red(
            `Error: app name "${name}" must be kebab-case (lowercase letters, digits, hyphens) and start with a letter.`
          )
        );
        process.exit(1);
      }
      ...
      console.log();
      console.log(
        chalk.dim("When ready: akari app certify (Phase 2b)")
      );
    });
}
