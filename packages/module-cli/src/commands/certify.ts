/**
 * certify.ts — `akari module certify` command (AKARI-HUB-024 §6.8 / §6.9)
 *
 ...
 *   4. Naming lint — module id, agent ids (naming.ts / ADR-011)
...
export async function runCertify(moduleDir: string): Promise<number> {
  const absDir = path.resolve(moduleDir);
  console.log(c.bold(`\nAKARI Module Certify — ${absDir}`));
  ...
  console.log(c.dim(`  Module: ${manifest.module.id} (${manifest.module.name}), tier=${tier}`));
  ...
  const namingResult = runNamingLint(manifest as { module: { id?: string }; agents?: Record<string, unknown> });
  ...
  const categoryResult = validateCategoryFromManifest(manifest as { module?: { category?: unknown } });
  ...
  if (overallPassed) {
    ...
    console.log(c.dim("  Fix the errors above and re-run: akari module certify"));
  }
  console.log();
...
/**
 * Register the `certify` sub-command on the `module` command object.
 * Called from cli.ts after the `module` Command is created.
 *
 * Usage: akari module certify [--dir <path>]
 */
export function registerCertifyCommand(moduleCmd: Command): void {
  moduleCmd
    .command("certify")
    .description(
      "Run Automated Lint + Contract Tests for the module in the current directory (AKARI-HUB-024 §6.8)"
    )
