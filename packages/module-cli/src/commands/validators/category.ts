/**
 * category.ts — Module category enum lint (ADR-013)
 *
 * Validates the [module] category field in akari.toml against:
 *   - Core 11 fixed enum (HUB-005)
 *   - or x-<kebab-slug> extension pattern
 *
 * Reference: ADR-013-capability-module-category-enum.md
 */
...
export function validateCategory(category: unknown): CategoryLintResult {
  ...
  if (category === undefined || category === null || category === "") {
    ...
    return { valid: true, errors: [], warnings: ["[module] category is not set — recommended for Marketplace filtering"], isCore: false, isExtension: false };
  }

  if (typeof category !== "string") {
    return {
      valid: false,
      errors: [{ field: "[module] category", message: "category must be a string", code: "CATEGORY_NOT_STRING" }],
      warnings,
      isCore: false,
      isExtension: false,
    };
...
export function validateCategoryFromManifest(manifest: {
  ...
  return validateCategory(manifest.module?.category);
}
