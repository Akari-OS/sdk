# `akari module certify` — 認定パイプライン詳細
...
`akari module certify` は Module を AKARI Marketplace や自己配布に向けて品質保証するための
...
| 層 | カバー | コマンド |
|---|---|---|
| Automated Lint | ✅ | `akari module certify` |
| Contract Test  | ✅ (スタブ) | `akari module certify` |
...
# ディレクトリを指定
akari module certify --dir ./my-module
```

**Exit code**:
- `0` — 全チェック PASS（CI で `akari module certify || exit 1` として使える）
- `1` — エラーあり
...
Step 4  命名規約 Lint（module id + agent ids）
Step 5  カテゴリ Lint
...
| `[module] id` | 文字列、必須 |
| `[module] name` | 文字列、必須 |
| `[module] tier` | `"full"` または `"mcp-declarative"`、必須 |
| `[module] category` | 推奨（未設定は warning） |
| `[permissions]` | セクション必須 |
...
ADR-011 の Module agent 命名規約を強制します：

| チェック | ルール |
|---|---|
| `[module] id` | 逆ドメイン形式 `^[a-z][a-z0-9]*(\.[a-z][a-z0-9_-]+)+$` |
| `[agents]` キー | `<module-short-id>_<role>` の snake_case |
| `<module-short-id>` | module id の最終セグメント、kebab → snake 変換 |
...
| `MISSING_MODULE_SECTION` | manifest | `[module]` セクションがない |
...
| `MODULE_ID_NOT_REVERSE_DNS` | naming | module id が逆ドメイン形式でない |
| `AGENT_ID_MISSING_PREFIX` | naming | agent id に module-short-id prefix がない |
...
`akari module certify` のルールを VSCode Extension の inline lint として提供。
保存のたびにリアルタイムでエラーを表示。
