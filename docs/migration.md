...
AKARI Module 開発者が直面するマイグレーションには大きく **2 種類** ある。
...
- 複数の Module 固有エージェントを Panel に密結合させる設計になった
...
Step 3: MCP サーバーの Module 内化（任意）
Step 4: akari module certify で昇格後の品質ゲートを通過
```
...
MCP-Declarative Tier では MCP サーバーが Module の外側（サイドカー）に存在していた。
...
#### Step 4: `akari module certify` の再実行
...
# 差分を確認（変更点だけハイライト）
akari module certify --diff-from=mcp-declarative
```
...
> Skill は他 Module から `skill.call("x-sender.post", {...})` で再利用できる。
...
| **Skill 登録** | 新たに `[skills.exposed]` を追加すれば他 Module に開放できる |
...
2. **Agent の大幅削減に同意できる** — Module 固有エージェントを持たない構成にする
...
```bash
# 1. 機能一覧の棚卸し
akari module inspect --list-capabilities
...
AKARI Module SDK は Semantic Versioning に準拠する。
...
Module の `akari.toml` での SDK 互換宣言:
...
互換範囲外の Core で Module が実行されようとした場合、**Core が実行を拒否**する（AC-7）。
...
AKARI Module SDK の Deprecation は、開発者が安心してエコシステムに参入できるよう、
以下の原則に従う。
...
| v0 に `deprecated` 警告 | T+1 ヶ月 | `akari dev` / `akari module certify` で警告 |
| v0 に `error` 格下げ | T+5 ヶ月 | 警告 → エラーに昇格（動作は継続） |
| v0 の公式サポート終了 | T+6 ヶ月 | Core の自動更新では v0 Module が起動拒否される可能性 |

> 上記スケジュールは v1.0 リリース時に確定する。v0 期間中は緩やかな運用を想定している。

### 5.3 Module 開発者の推奨アクション

```bash
# 現在の SDK バージョンと互換状況を確認
akari module inspect --sdk-compat

# 推奨される最小バージョンへの更新
npm install @akari/sdk@latest

# breaking change の影響範囲を事前チェック
akari module certify --check-compat
```
...
1. **「まず MCP-Declarative で始める」は投稿系 Module に特に有効** — テキスト入力 + 添付 + HITL ボタンは Panel Schema v0 の標準 widget で十分カバーできる
...
```bash
# 宣言漏れを Lint で検知
akari module certify --lint-only
