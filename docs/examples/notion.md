...
> **対象 spec**: `spec-module-notion-reference.md (AKARI-HUB-026, Hub)`（AKARI-HUB-026 v0.1.0）
> **Panel Schema**: `notion-module-panel.schema.json (Hub)`
...
Notion Module は **Notion workspace の操作全般**を担当する Documents Module。
...
`akari module certify` のContract Test がこれを定期的に検出する（CI で回す）。
...
X Sender の `amp = ["write"]`（書き込み専用）と違い、Notion Module は `read` も要求する。
...
Panel Schema のファイルは `notion-module-panel.schema.json (Hub)`。
...
spec §6 を読む。Notion Module の最大の付加価値は他 Module との連携（Cross-over）。
...
Notion Module の各操作は AMP に記録される。`kind` フィールドで操作種別を表す。
...
Notion Module は `kind: "export-action"` を記録することで、
「この下書きがどの Notion ページになったか」のトレースが AMP から辿れる。

---

## 10. 真似するポイント — 他 Documents Module を作るとき

Notion Module は **Google Docs / Microsoft 365 / Coda** を作る際の雛形として設計されている。
...
新しい Documents Module を作るときに確認する項目:
...
| リソース | パス |
|---|---|
| HUB-026 spec 本体 | `spec-module-notion-reference.md (AKARI-HUB-026, Hub)` |
| Panel Schema JSON | `notion-module-panel.schema.json (Hub)` |
