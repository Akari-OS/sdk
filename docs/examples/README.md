# AKARI Module SDK — Example Gallery

> **このドキュメントについて**: AKARI Module SDK の参考実装（Example）を一覧化したギャラリー入口。
...
2. **次に [notion.md](./notion.md) を読む**
   ...
   - オフライン挙動（キャッシュ + 書き込みキュー）は他の Documents Module でも再利用できる設計
...
| Inter-App handoff（Module 間連携）を学びたい | x-sender.md §6 Writer handoff / notion.md §8 Cross-over |
...
| 自分で Publishing Module を作りたい（LINE / Threads など） | x-sender.md §10 真似するポイント |
| 自分で Documents Module を作りたい（Google Docs など） | notion.md §10 真似するポイント |

---

## 参考実装が示す設計パターン

AKARI の 2 本の参考実装は、Module 開発者が知るべき **共通の設計パターン** を提示している。
...
Module 開発者は **ビジネスロジック（MCP tools）と UI 宣言（Panel Schema）** だけに集中できる。
...
ユーザーが最終確認できるよう設計する。`akari module certify` がこれを静的検知する。
...
Module 間の連携では、実データではなく **AMP / Pool の参照 ID** だけを渡す。
...
Module コードが credential の文字列を直接保持することは禁止。
...
| notion.md | `spec-module-notion-reference.md (AKARI-HUB-026, Hub)` | `notion-module-panel.schema.json (Hub)` |
...
1. `` (Hub) に `spec-{module-name}.md` を作成し、`akari module certify` で Pass させる
2. このギャラリーに `{module-name}.md` の読み方ガイドを追加する
...
1. Module 概要（Tier / カテゴリ / 公式 MCP / Phase）
2. `akari.toml` の読みどころ
...
| ドキュメント | 場所 | 説明 |
|---|---|---|
| Module SDK spec | `Module SDK spec (AKARI-HUB-024, Hub)` (HUB-024) | Tier 定義・Certification・7 API 群 |
| Panel Schema spec | `Panel Schema spec (AKARI-HUB-025, Hub)` (HUB-025) | UI 宣言規格・widget 一覧 |
| Declarative Modules spec | `spec-akari-declarative-capability-modules.md (AKARI-HUB-005, Hub)` (HUB-005) | カテゴリ定義・Publishing / Documents の全 Module 一覧 |
