...
> MCP-Declarative Tier で AKARI App が成立することを実証する。

---

## 1. App 概要

X Sender は **X (旧 Twitter) への投稿** を担当する Publishing App。
...
**最大の特徴**: App 開発者が書くものは `x-mcp` サーバーと `panel.schema.json` の **2 ファイルのみ**。
...
`akari app certify` の静的 Lint でも検証される。
...
`akari app certify` は Panel Schema の `bind` 宣言と、この `required` / `properties` の整合性を自動検証する。
...
この方式は「App コードが credential に直接アクセスできる」ため、
悪意のある App が他の App のトークンを盗めるリスクがあった。
...
spec §11 の AC リストは単なるチェックリストではなく、**App の品質保証仕様**でもある。

注目すべき AC をいくつか抜粋する:

```
AC-1: akari app certify が Pass する
...
**AC-1 と AC-11** は `akari app certify` による自動検証で担保される静的チェック。
...
## 10. 真似するポイント — 他 Publishing App を作るとき
...
新しい Publishing App を作るときに確認する項目:
...
| Notion App ガイド | [notion.md](./notion.md)（Documents カテゴリの参考実装） |
