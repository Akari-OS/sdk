---
title: Getting Started — 最初の Module を 5 分で作る
updated: 2026-04-19
related: [HUB-024, HUB-025, HUB-007]
---

# Getting Started — 最初の Module を 5 分で作る / Build Your First Module in 5 Minutes

> MCP-Declarative Tier で「Hello Module」を作り、AKARI Shell で動かすまでのチュートリアル。
...
| `@akari/sdk` CLI インストール済み | `npx akari-module-cli --version` |
| OAuth アプリ作成（外部サービス連携時のみ） | 各サービスの開発者コンソール |

> **Note**: 本チュートリアルはネットワーク不要のデモ Module を作る。
> 外部 API 連携は [Cookbook](./cookbook/) を参照。

---

## Step 1: 雛形生成 / Scaffold the Module

```bash
npx akari-module-cli create my-first-module --tier mcp-declarative
cd my-first-module
```
...
```toml
[module]
id       = "com.yourname.my-first-module"   # ← 逆ドメイン記法
...
server.tool(
  ...
  async ({ name, goal_ref }) => {
    ...
    await amp.record({
      ...
      goal_ref: goal_ref ?? "my-first-module-demo",
    })
...
AKARI Shell が起動し、`my-first-module` が自動で mount される。

```
[akari dev] Watching: mcp-server/index.ts, panels/main.schema.json
[akari dev] Module mounted: com.yourname.my-first-module
[akari dev] Panel: Hello (main) ← Shell の左サイドバーに表示
```

Shell の左サイドバーから "Hello Module" を開き、名前を入力して "挨拶する" ボタンをクリック。
...
## Step 6: `akari module certify` で Lint を実行する / Run Certification Lint
...
最初の Module が動いた。次にどこへ進むか：

| 目標 | 行き先 |
|---|---|
| Full か MCP-Declarative か詳しく知りたい | [Tier Comparison](./concepts/tier-comparison.md) |
| Module のライフサイクルを理解したい | [Module Lifecycle](./concepts/module-lifecycle.md) |
...
> 実際のサービス連携では [X Sender 参考実装](../examples/x-sender/)
> または [Notion 参考実装](../examples/notion/) を参照。
