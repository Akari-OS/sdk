...
> Module を書くとき「自分のコードがどこで実行され、何と話しているか」を理解するガイド。
...
│  │  (Full)    │  │  (Full)    │  │ (MCP-Decl) │  │  Module      │  │
│  └──────┬─────┘  └──────┬─────┘  └──────┬─────┘  └──────┬───────┘  │
│         └───────────────┴───────────────┴───────────────┘           │
│                     ↕ AKARI Module SDK（@akari/sdk）                 │
├─────────────────────────────────────────────────────────────────────┤
│  AKARI Core                                                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐  │
...
Module 開発者が直接触るのは主に「Module 層」と「AKARI Module SDK（@akari/sdk）」の境界。
Core の内部実装を直接触ることはない（SDK 経由で全て行う）。

---

## Module → Shell: パネルの mount フロー / Panel Mount Flow

Module が Shell に Panel を表示するまでの流れ。
...
akari.toml の [panels] セクション
  ...
  ↓ Schema レジストリに登録（module-id → schema のマッピング）
...
## Module → Agent Runtime: エージェントの呼び出しフロー / Agent Invocation Flow

Module がカスタムエージェントを呼び出すとき（Full Tier のみ）。
...
- 状態は Module にもエージェントにも持たせず、**Memory Layer（Pool / AMP）のみ**

---

## Module → Memory Layer: 記憶の読み書き / Memory Read/Write

全ての状態は Memory Layer に書く。Module 本体・MCP サーバー・エージェントのどれも自分の中に状態を持たない。
...
## Module 間の通信: handoff フロー / Inter-Module Communication

Module 同士は**直接通信しない**。全て記憶層（Pool / AMP）経由。

```
[Writer Module が X Sender に下書きを渡す]
...
Shell
  ...
  ↓ handoff intent を X Sender に伝える
...
AKARI の 4 プロトコルは、SDK を通じて Module から透過的に使われる。
...
Module 開発者が直接 MCP/M2C/AMP/ACE のプロトコル仕様を読む必要はない。
...
| 層（内向き） | Module SDK から見た役割 | あなたが触る部分 |
|---|---|---|
...
Module 開発者が「直接触る」のは Shell / Agent Runtime / Memory Layer の 3 層。
...
- [HUB-024 §6](https://github.com/Akari-OS/.github/blob/main/VISION.md) — Module SDK 完全仕様
