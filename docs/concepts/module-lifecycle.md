---
title: Module Lifecycle — インストールから廃止まで
updated: 2026-04-19
related: [HUB-024, HUB-005]
---

# Module Lifecycle — インストールから廃止まで / From Install to Uninstall

> Module は「インストール → 有効化 → 開発 → 公開 → 更新 → 無効化 → アンインストール」の
> ライフサイクルを持つ。各フェーズで Core（Shell / Agent Runtime / Memory Layer）との
> 関係がどう変わるかを理解することで、正しい Module 設計ができる。
...
```bash
# 公式マーケットから
akari module add com.akari.x-sender

# npm パッケージとして（自己配布）
akari module add @myorg/my-module

# Git リポジトリから直接
akari module add https://github.com/myorg/my-module
```
...
```
Error: Module "com.example.my-module" requires sdk ">=2.0.0",
       but current Core SDK is "0.4.2".
       Update AKARI Core to install this module.
```
...
Module の通常稼働状態。**CAA（Costume Agent Architecture）の揮発/永続原則**がここで機能する。
...
| Pool・AMP（記憶層） | **永続** | Module が終了しても残る。再起動後も参照可能 |

この分離があるため：

- Module がクラッシュしても**ユーザーのデータ（記憶層）は消えない**
- 複数の Module が同時に Pool / AMP にアクセスできる
...
```bash
akari module update com.example.my-module
```
...
- Pool / AMP のデータは引き継がれる（Module がデータ構造を変えた場合は移行処理が必要）
...
```bash
akari module disable com.example.my-module   # 無効化（データ保持）
akari module remove  com.example.my-module   # 完全削除
```
...
> **設計の意図**: Module を削除しても、そのツールを使って作ったコンテンツ（Pool）や
> 操作履歴（AMP）は消えない。これにより、Module を入れ替えても**過去の作業の連続性が保たれる**。
...
Module のライフサイクルはこの原則の**道具層（MCP サーバー）への適用**そのもの。

```
思考層（揮発）: エージェント
道具層（揮発）: MCP サーバー（Panel を閉じると消える）← Module の主体
記憶層（永続）: Pool + AMP ← Module が書いたデータは残る
```
...
3. **Pool / AMP の ID を大切にする** — Inter-App handoff の根拠になる
...
A: 消えない。Pool はユーザーのデータ。Module はそこに書き込む権限を持つが、
...
- [HUB-024 §6.2](https://github.com/Akari-OS/.github/blob/main/VISION.md) — Module Tier 仕様
