...
> **対象**: AKARI Module SDK（HUB-024）で外部サービスと連携する Module 開発者
...
AKARI Module が外部サービスと連携するための認証パターンを 3 種類解説する。
...
# [oauth.google.credentials] は Module の環境変数で別途注入
```
...
await amp.record({
  ...
  goal_ref: handoffGoalRef ?? `${moduleId}:${Date.now()}`,
  metadata: {
    target:       "x",
    tweet_id:     tweetId,
    tweet_url:    tweetUrl,
...
export async function withAuth<T>(
  fn: () => Promise<T>,
  context: { moduleId: string; goalRef?: string }
): Promise<T> {
  try {
    return await fn()
  } catch (err) {
...
async function handleAuthError<T>(
  ...
  if (isExpiredTokenError(err)) {
    try {
      await refreshAccessToken({ service: context.moduleId } as TokenStore)
  ...
  if (err instanceof OAuthRefreshError &&
    ...
    await amp.record({
      kind: "auth-error",
      content: `${context.moduleId}: 再認証が必要です`,
      goal_ref: context.goalRef ?? context.moduleId,
    ...
    await shell.notify({
      title: "再認証が必要です",
      body: `${context.moduleId} への接続が切れました。クリックして再認証してください。`,
      action: { type: "open-settings", moduleId: context.moduleId },
    })
...
export async function postToX(params: {
  ...
  return await withAuth(
    ...
    { moduleId: "com.akari.x-sender", goalRef: params.goalRef }
  )
}
```
...
以下をすべて満たすこと。満たさない Module は `akari module certify` の Lint で reject される。
