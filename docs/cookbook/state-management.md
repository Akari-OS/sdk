...
> - Module 再起動時の state 復元フロー

---

## AKARI の state 設計哲学

AKARI では **「Module 本体に state を持たない」** が大原則（HUB-024 §6.7 Guideline 9）。

> Module が終了・再起動・クラッシュしても、ユーザーの作業は失われない。
> なぜなら state の実体は**記憶層（Pool / AMP）**にあるから。

とはいえ「UI の表示状態」「フォームの入力中値」「ページネーションのカーソル」など、
永続化が不要な一時的 state は Module 側に持って構わない。

この 2 種類を**明確に分離**することが state 管理の核心。

| state の種類 | 保存先 | 対象 | 再起動時 |
|---|---|---|---|
| **Ephemeral（揮発）** | Module メモリ内 | フォーム入力中・UI の開閉状態・ページカーソル | リセットされる（それでよい） |
...
Full Tier Module は React Panel を持つため、状態管理ライブラリを選択できる。
...
type ModuleStore = EphemeralState & PersistentState
...
export const useModuleStore = create<ModuleStore>()(
  devtools(
    ...
    { name: "ModuleStore" }  // Redux DevTools の表示名
  )
)
```
...
### Zustand の slice パターン（大きな Module 向け）

Module が複数の関心事を持つ場合は slice に分割する：
...
// src/store/module-store.ts（slice を合成）
import { create } from "zustand"
...
type ModuleStore = PoolSlice & DraftSlice

export const useModuleStore = create<ModuleStore>()((...args) => ({
...
受信 Module の handler が返す `initial_state` は、`state.*` binding に注入される：
...
`module.save_draft` の MCP tool 実装内で `amp.record()` を呼ぶことで永続化する。
...
const charCount = useModuleStore((s) => s.draftText.length)

// 文字数制限チェック（derived + computed）
const isOverLimit = useModuleStore((s) => s.draftText.length > 280)

// 複数 state から derived な値を計算する場合
import { useCallback, useMemo } from "react"

function usePostValidation() {
  const { draftText, selectedTags, mediaIds } = useModuleStore()
...
## Part 5: Module 再起動時の state 復元

Module は起動時（Shell が MCP サーバーを起動したとき）に
...
server.tool(
  "module.initialize",
  "Module の初期状態を返す（起動時 / Panel 開封時に Shell が呼ぶ）",
...
export function WriterPanel() {
  const { loadDraft, setDraftText } = useModuleStore()
...
3. **復元失敗は graceful に**: AMP が空でも Module は動作する（空の初期状態で起動）
...
| チェック項目 | OK | NG |
|---|---|---|
| bytes / 大きな文字列を Module state に持っていない | Pool/AMP に保存し ID のみ保持 | `state.image = new Uint8Array(...)` |
