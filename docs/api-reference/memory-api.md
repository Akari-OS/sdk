...
> Module は自前の DB を持たない。全データはこの API 経由で読み書きする。
...
- Module は **自前 DB を持たない**（Guidelines §2、HUB-024 §6.7-2）
...
Tiered Storage (internal spec) で定義された Hot/Warm/Cold の Tiered Storage 上に実装されており、Module からは透過的に見える。
...
- tier ポリシーに従い Hot/Warm/Cold のどこかに格納される（Module は意識しない）
...
Pool は内部で 3 層の Tiered Storage を持つが、**Module から見ると透過的**。
...
```typescript
// tier の確認（情報として参照するのみ。Module が tier を直接変更しない）
...
interface AmpRecordInput {
  ...
  /**
   * ゴール参照（必須）
   * Module ID または ADR-011 で定義された ref 形式（§4 参照）
   */
  goal_ref: string
...
Module が `memory.amp.record()` で記録する際は、`confidence` を省略すると `kind` に応じたデフォルト値が使われる。
...
| kind | AMP type | 用途 | 典型 Module |
|---|---|---|---|
| `goal` | episodic | ゴール・意図の記録 | 全 Module |
...
| `decision` | episodic | 意思決定の記録 | 全 Module |
| `style-preference` | semantic | ユーザーのスタイル・好みの学習 | Writer / Video |
| `error` | episodic | エラー・失敗の記録 | 全 Module |
| `working` | working | セッション内の一時的な作業状態 | 全 Module |

**カスタム kind**: `Module ID + ドット + 名前` の形式で独自 kind を定義できる。

```typescript
// カスタム kind の例
kind: "com.myorg.mymodule.custom-event"
```
...
### 4.1 Module ID を ref として使う

**最も一般的な用途**: Module の動作ログを記録する場合、`goal_ref` に Module ID を使う。

```typescript
// Module Manifest (akari.toml) の [module].id を使う
goal_ref: "com.akari.writer"
goal_ref: "com.x.sender"
goal_ref: "com.myorg.mymodule"
```
...
Module 間 handoff（HUB-024 §6.6-5）では、AMP record の `goal_ref` を引き継ぐ。
...
Module が `pool.get()` を呼ぶと、Cold 素材の場合は `tier: "cold"` かつ bytes が空の intermediate response が返ることがある。
...
Module はプロバイダーを直接指定しない。ユーザーが OS 設定で構成し、Module には透過的に注入される。
  ...
  if (e instanceof MemoryError) {
    switch (e.code) {
      ...
      case MemoryErrorCode.PermissionDenied:
        // -34002: 他 Module の private 記憶にアクセス不可
...
| `-34002` | `PermissionDenied` | アクセス権限なし（他 Module の private 記憶等） |
...
type AmpKind =
  ...
  | "error"
  | "working"
  | string  // カスタム kind: "com.org.module.event-name"
...
### 例 4: Module 間引き継ぎ — Writer から Video へ

```typescript
import { memory, module } from "@akari/sdk"

// Writer Module: 下書きと関連素材を Video に渡す
...
3. **Inter-App は ID のみ渡す** — `module.handoff()` に bytes を直接渡さない（Pool の ID を渡す）
4. **小さく記録して reinforce する** — 大きな 1 件より、小さな複数件 + reinforce のほうが AMP の decay に強い
5. **Cold tier を意識しない** — Tier 管理は Core の責任。Module は `pool.get()` を呼ぶだけ
...
| [Inter-App API](./inter-app-api.md) | `module.handoff()` で Pool / AMP の ID を Module 間で受け渡す |
| **仕様書** | |
| [HUB-024 §6.6-2](https://github.com/Akari-OS/.github/blob/main/VISION.md) | Memory API の Module SDK 上の位置づけと原則 |
