...
Permission API は、Module が**機密操作を実行する前にユーザー承認を取る**契約を定義する。
AKARI のセキュリティモデルは **最小権限原則（Principle of Least Privilege）** に基づく。

### 最小権限原則

- Module は `akari.toml` で宣言した scope 以外の操作を要求できない
- 未宣言の scope は実行時に自動的に拒否される（デフォルト deny）
- AKARI Core がすべての承認要求を仲介する（Module が直接ユーザーに問い合わせない）
...
| Module 間 | `inter-app:com.akari.video` | 特定 Module への handoff |
...
Module は `akari.toml` の `[permissions]` セクションで**必要な scope をすべて事前宣言**する。
...
# Module 間 handoff（handoff 先の Module ID を列挙）
...
| `inter-app` | Module ID（逆ドメイン記法）の配列 | handoff 先として許可する Module |
...
Module からの revoke 要求は「ユーザーが自ら許可を取り消す」UI 操作に対応する
（Module が一方的に自分の権限を削除するためではない）。
...
| scope | 説明 | HITL 推奨 |
|---|---|---|
| `inter-app:<module-id>` | 指定 Module への handoff 送信（例: `inter-app:com.akari.video`） | No（受信側が判断） |
...
`<key>` は Core が管理するシンボリックなパスキー（実パスをModule に公開しない）。
...
Runtime API の `permission.gate({ hitl: true })` は Module のビジネスロジックが処理する。

**MCP-Declarative Tier** では、Shell が `hitl.require: true` を検出すると、
MCP ツール呼び出しの前に自動的に承認ダイアログを挿入する。
Module 側で `permission.gate()` を明示的に呼ぶ必要はない（Shell が代行）。

**Full Tier** では、Module のコード側で `permission.gate({ hitl: true })` を明示的に呼ぶ。
...
Module 本体（Panel / Agent コード）はトークンの実体に直接アクセスしない。
...
承認ダイアログは Shell が管理し、Module は UI を自前で用意しない。
...
```
┌─────────────────────────────────────────────────────────┐
│ [Module アイコン] X Sender が許可を求めています         │
│                                                         │
│ 操作: X（Twitter）に投稿                                │
│ 権限: mcp:x.post                                        │
│                                                         │
...
| フィールド | 内容 | 出所 |
|---|---|---|
| Module 名 + アイコン | どの Module が要求しているか | `akari.toml` の `[module]` |
...
| ポリシー | 動作 | 用途 |
|---|---|---|
| `"always"` | 同じ Module + scope の組み合わせは今後常に自動承認 | 日常的に使う操作（下書き保存など） |
...
ユーザーは AKARI Shell の設定画面（`設定 > Module > <Module名> > 権限`）から
個別ポリシーをリセットできる。
Module コード側からポリシーを直接書き換えることはできない。
...
Module が明示的に記録する必要はない（Core が代行する）。
...
// 自モジュールの permission ログを取得
const auditLog = await amp.query({
  kind: "permission-grant",
  filter: { module_id: "com.x.sender" },
})
```

> **注意**: Module が照会できるのは**自モジュールのログのみ**。
> 他 Module のログへのアクセスは `amp:read` があっても制限される。
...
X（Twitter）への投稿を行う MCP-Declarative Module の実装例。
...
Notion への書き込みと削除を行う Module の実装例。
...
| **Module SDK spec** ([HUB-024](https://github.com/Akari-OS/.github/blob/main/VISION.md)) | Permission API の仕様の正典（§6.6）、`[permissions]` 宣言の原典（§6.5） |
