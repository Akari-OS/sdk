...
> **Module 間で "id を渡す"**。直接オブジェクトは渡さない。
...
Inter-App API は、AKARI Module 間の **handoff（受け渡し）** を実現する API。
`@akari/sdk` の `module` オブジェクトを通じて利用する。

### "id を渡す" 原則

**Module 同士は直接通信しない。Pool / AMP の ID のみを渡す。**
...
2. **バウンダリ保護**: bytes を直接受け渡すと Module 間に隠れた結合が生まれる。ID 渡しにより結合を断ち切る
3. **一貫した記憶層**: データの実体は常に Pool / AMP にある。Module 本体は state を持たない

---

## 2. `module.handoff(targetId, payload)`

送り側 Module が呼ぶメインの関数。
...
await module.handoff(targetId: string, payload: HandoffPayload): Promise<HandoffResult>
```
...
| 引数 | 型 | 説明 |
|---|---|---|
| `targetId` | `string` | 受け取り先 Module の ID（例: `"com.akari.x-sender"`）または category セレクタ（§6 参照） |
...
const result = await module.handoff("com.akari.x-sender", {
  kind: "publish-draft",
  ref: {
...
interface HandoffHints {
  ...
  [key: string]: unknown      // Module 固有のヒント
}
```
...
- `hints` は受け側 Module が無視しても動作するよう、あくまで「ヒント」として設計すること

### kind 一覧（公式 Module が使う標準 kind）

| kind | 送り元の典型例 | 受け側の典型例 |
|---|---|---|
| `publish-draft` | Writer | Publishing カテゴリ Module |
| `export-to-doc` | Writer / Research | Documents カテゴリ Module |
...
受け取り側 Module は `module.onHandoff(handler)` でハンドラを登録する。
...
module.onHandoff(handler: HandoffHandler): void
...
### 実装例（Publishing Module 側）

```typescript
import { module, amp, pool } from "@akari/sdk"

module.onHandoff(async (handoff) => {
...
```typescript
// Writer 側（送り元）
import { pool, amp, module, permission } from "@akari/sdk"

async function handoffToPublishing(content: string, targetModule: string) {
  // 1. 下書きを Pool に保存
  ...
  // 4. handoff（ID のみ渡す）
  const result = await module.handoff(targetModule, {
    kind: "publish-draft",
    ref: {
      draft: draftPoolId,
      ampRecordId,
...
Research Module が収集した情報を Google Sheets や Excel に出力する。

```typescript
// Research Module 側（送り元）
import { amp, module } from "@akari/sdk"
...
async function exportToSheets(searchResults: SearchResult[]) {
  ...
  await module.handoff("com.akari.gsuite", {
...
```typescript
// Video Module 側（送り元）
import { pool, amp, module } from "@akari/sdk"
...
async function exportToPresentationSlides(
  ...
  await module.handoff("com.akari.m365", {
...
```typescript
// 特定の Module を指定
await module.handoff("com.akari.x-sender", payload)
await module.handoff("com.akari.notion", payload)
await module.handoff("com.third-party.my-module", payload)
```

### Category セレクタ

インストールされた Module の category をもとに Core が解決する。

```typescript
// "publishing" カテゴリの Module を選ばせる
await module.handoff("category:publishing", payload)

// "documents" カテゴリの中で targetOutlet に最も近い Module
await module.handoff("category:documents", {
  kind: "export-to-doc",
  ref: { draft: draftId },
...
1. `hints.targetOutlet` に一致する Module がある → その Module を選択
2. 候補が複数ある → Shell がユーザーに選択プロンプトを表示
3. 候補が 0（未インストール） → `HandoffResult.status = "not-installed"` を返す

### Module が未インストールの場合のフォールバック

```typescript
const result = await module.handoff("com.akari.notion", payload)
...
if (result.status === "not-installed") {
  ...
  // または: ユーザーにインストールを提案
  shell.notify({
    message: "Notion Module がインストールされていません",
    action: { label: "インストール", moduleId: "com.akari.notion" },
  })
}
```
...
Module が `module.handoff()` を呼ぶと、Core は以下を AMP に書き込む：

```typescript
// Core が自動で記録する（Module 側の実装不要）
{
  kind: "handoff",
...
```typescript
import { permission, module } from "@akari/sdk"
...
switch (result.status) {
  ...
  case "not-installed":
    // Module がインストールされていない
    console.warn("対象 Module が未インストール:", result.reason)
    // フォールバック処理...
    break

  case "rejected":
    // Module が受け取りを拒否（型不一致・権限不足等）
...
```typescript
// kind が間違っている例
await module.handoff("com.akari.x-sender", {
...
`module.handoff()` はデフォルト 30 秒でタイムアウトする。
...
/** module オブジェクト（@akari/sdk からインポート） */
...
import { pool, amp, ace, module, permission, shell } from "@akari/sdk"
...
export async function publishToX(
  ...
  if (result.status === "accepted") {
    ...
    shell.notify({
      ...
      action: { label: "インストール", moduleId: "com.akari.x-sender" },
    })
  }
}
```
...
export async function exportToNotion(
  ...
  const result = await module.handoff("category:documents", {
    kind: "export-to-doc",
    ref: {
      draft: articleId,
      ampRecordId: ampId,
  ...
  if (result.status === "not-installed") {
    // Notion Module なし → フォールバックで clipboard に
...
| [VISION.md — Module 間の行き来](../../../VISION.md) | handoff 原則の設計思想 |
