...
> **対象読者**: Module 開発者（Full Tier）
> **前提知識**: [`Module SDK spec (AKARI-HUB-024, Hub)`](https://github.com/Akari-OS/.github/blob/main/VISION.md) §6.6(1)、[ADR-011 (Hub)]
> **関連ガイド**: Memory API / Permission API / Inter-App API

Agent API は、Module が **Agent Runtime**（AKARI Core 内の実行環境）と対話するための契約です。
Module はこの API を通じて、OS 標準の 7 体 reference defaults を呼び出したり、
...
8. [Handoff — Module 間で agent を渡す](#8-handoff--module-間で-agent-を渡す)
...
│   ・Module-supplied agents（登録した独自 agent）│
...
| persona を切り替える | `agent.switchPersona(moduleId)` | Module 切替時にパートナーの人格を差し替える |
| ハンドオフする | `agent.handoff(id, payload)` | 別 Module の agent に制御を渡す |

### Full Tier 限定

Agent API の `register` / `spawn` / `handoff` は **Full Tier Module のみ**利用できます。
...
[agents]
writer_editor = "agents/editor.md"       # ADR-011: <module-short-id>_<role>
writer_reviewer = "agents/reviewer.md"
```

> **命名規約（ADR-011）**: エージェント ID は `<module-short-id>_<role>` の snake_case 形式。
> `module-short-id` は `akari.toml` の `[module] id` の末尾セグメントを snake_case 化したもの。
...
AKARI Core が標準で提供する 7 体（reference defaults）は、どの Module からでも呼び出せます。
...
// ペルソナ付き（現在の Module コンテキストを伝える）
const reply = await agent.reference.partner.ask(
  "この投稿のフックを3パターン出して",
  {
    conversationId: "writer-session-abc",
    moduleContext: "Writer モード — SNS コピーライティング",
  }
)
```
...
Module 固有のエージェントを Agent Runtime に登録します。
...
| 名前 | 型 | 必須 | 説明 |
|---|---|---|---|
| `id` | `AgentId` | ✅ | `<module-short-id>_<role>` の snake_case（ADR-011） |
...
エージェント ID は必ず **`<module-short-id>_<role>`** の形式にしてください。

```
com.akari.writer  →  module-short-id = writer
  writer_editor           ✅
  writer_reviewer         ✅
  editor                  ❌ (prefix なし)
...
Manifest バリデータは `<module-short-id>_` prefix を自動チェックします。
...
## 8. Handoff — Module 間で agent を渡す

Handoff は、あるエージェントの実行コンテキストを別の Module のエージェントに引き継ぐ操作です。
Inter-App API の `module.handoff()` と連携して使います。
...
| `payload.fromModule` | `string` | ✅ | 送信元 Module ID |
| `payload.toModule` | `string` | ✅ | 送信先 Module ID |

### 使用例 — Writer から Video へ

```typescript
import { agent, pool, amp, module } from "@akari-os/sdk"
...
await agent.handoff("video_director", {
  ...
  fromModule: "com.akari.writer",
  toModule: "com.akari.video",
})

// 4. Inter-App API でモジュールを切り替える
...
この Note は次のモジュールの系プロンプトに自動注入され、「さっきの下書きを踏まえた動画ですね」と
即座に対応できるようになります。

```typescript
// Handoff Note の構造（自動生成されるが内容を確認できる）
interface HandoffNote {
  fromModule: string        // 送信元 Module
...
Module に合わせた専門性を持たせられます。

これは Module が Shell に mount される際に自動で呼ばれますが、手動で呼び出すこともできます。

### `agent.switchPersona(moduleId, options?)`
...
| 名前 | 型 | 必須 | 説明 |
|---|---|---|---|
| `moduleId` | `string` | ✅ | 切り替え先 Module の ID |
...
```typescript
// Module が Mount された時点でパートナーのペルソナを切り替える
...
着ぐるみ = system prompt + ツールセット
  │
  ├── パートナー素（デフォルト）: 何も Module が入っていない状態
  ├── パートナー（Writer）:     Writer Module が mount された状態
  ├── パートナー（Director）:   Video Module が mount された状態
  └── ...
...
| `AGENT_INVALID_SPEC` | 400 | `id` が `<module-short-id>_<role>` 形式に準拠しているか確認 |
...
/** Module-supplied agent の ID: <module-short-id>_<role> の snake_case */
type AgentId = string
...
Writer Module がパートナーに下書き改善を依頼するシナリオです。

```typescript
// modules/writer/src/editor-panel.tsx
...
Publisher Module がオペレーターに SNS 投稿を依頼するシナリオです。
HITL（Human-in-the-loop）ゲートを通してから実行します。

```typescript
// modules/publisher/src/post-action.ts
...
### 例 3: Module 固有の writer_editor を使う

Writer Module が自前の `writer_editor` エージェントを登録・呼び出すシナリオです。

```typescript
// modules/writer/src/index.ts — Module 初期化
import { agent } from "@akari-os/sdk"
...
```typescript
// modules/writer/src/editor-panel.tsx — 実際の呼び出し
import { agent } from "@akari-os/sdk"
...
| パターン | 正誤 | 理由 |
|---|---|---|
| `writer_editor` | ✅ | `com.akari.writer` → prefix `writer_` |
| `pdf_reader_extractor` | ✅ | `com.third.pdf-reader` → prefix `pdf_reader_` |
| `editor` | ❌ | prefix なし |
