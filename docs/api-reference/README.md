...
AKARI Module SDK が提供する **7 つの API 群**の入口ページ。
...
AKARI OS は **Core（基盤）** と **Module（アプリ）** の 2 層で構成される。
Module 開発者は Core の機能を直接触らず、**Module SDK（`@akari-os/sdk`）** を通じてのみアクセスする。
...
│  Module（あなたのアプリ）                                            │
│                                                                     │
│   import { pool, amp, ace, shell, module,                           │
...
**Module から Core へのアクセスは必ずこの SDK 経由**。Core の内部実装に直接依存するコードは書かない（Guidelines §6.7 準拠）。
...
|---|---|---|---|
| **Agent API** | Module 固有エージェントの定義・呼び出し | Memory API（状態の保存）、Context API（文脈の組み立て） | どの Module でも使う |
| **Memory API** | Pool（素材）と AMP（記憶・判断ログ）へのアクセス | Core Memory Layer | どの Module でも使う |
| **Context API** | ACE を使ったコンテキストの組み立て・Lint | Memory API（ソースの取得） | Agent を呼び出す前 |
| **UI API** | Shell へのパネル mount、イベント受信 | Shell（Core） | Full Tier Module |
| **Inter-App API** | Module 間の handoff（データ受け渡し） | Memory API（ID 渡し） | Module をまたぐ連携 |
| **Permission API** | 権限ゲート・HITL（人間承認）の発火 | Core 権限管理、AMP（監査ログ） | 外部通信・HITL が要る操作 |
| **Skill API** | 関数の公開と他 Module からの呼び出し | Core Skill Registry | Module 間の機能共有 |
...
Skill API ──→ Core Skill Registry ──→ 他 Module
```
...
Module 固有のエージェントを定義し、ephemeral（揮発）に呼び出す API。`defineAgent` で仕様をファイルに固定し、`invoke` / `spawn` で実行する。エージェント自身は状態を持たず、状態は必ず Memory API に書く。
...
**Pool**（素材の Content-Addressed ストア）と **AMP**（記憶・判断ログ）への読み書き API。Module は自前の DB を持たず、すべてのデータをここに置く。AMP の全 record には `goal_ref`（ゴール参照）が必須。
...
Shell にパネルを mount し、Shell からのイベント（選択変化・フォーカス等）を受信する API。Module は独自ウィンドウを作らず、Shell の panel 規格（spec-shell-panel-framework）に従う。Full Tier では任意の React コンポーネントを渡せる。
...
Module 間でデータを受け渡す（handoff）API。直接 payload を送らず、Pool / AMP の **ID だけ**を渡す。受け側は ID から Memory Layer で素材を fetch する。これにより「誰が何を渡したか」が AMP に自動記録される。
...
関数を他の Module に公開（`skill.register`）し、他 Module の関数を呼び出す（`skill.call`）API。Input / Output は JSON Schema で型付け必須。公開 skill は Certification で review 対象になる。
...
import { pool, amp, ace, shell, module, permission, skill } from "@akari-os/sdk"
```
...
| **他の Module にデータを渡したい** | [Inter-App API](./inter-app-api.md)（`module.handoff`） | bytes ではなく Pool / AMP の ID を渡す |
| **外部 API を叩く前に承認を取りたい** | [Permission API](./permission-api.md)（`permission.gate`） | `hitl: true` で承認 UI が出る |
| **自分の機能を他 Module に公開したい** | [Skill API](./skill-api.md)（`skill.register`） | JSON Schema で Input / Output を型付け |
| **他 Module の機能を呼び出したい** | [Skill API](./skill-api.md)（`skill.call`） | `akari.toml` の `[skills.imported]` に宣言 |
...
import type {
  ...
  HandoffPayload,   // module.handoff の引数
...
Rust Module を開発する場合は `akari-sdk-rs` crate を使う（API の構造は同一）。
...
典型的なフロー：**Module が UI を出し、ユーザー承認を取り、エージェントを動かし、結果を保存し、他 Module へ渡す**。

```
Module の処理フロー（例: Writer が下書きを仕上げて X Sender に渡す）
...
- bytes ではなく ID だけが Module 間を流れる（Inter-App の原則）
...
Module の実装方式（Full / MCP-Declarative）については Tier ガイドを参照。
...
- [AKARI-HUB-024 Module SDK (正典は Hub)] — 本 API 群の原仕様
- Shell + Module モデル (internal spec)
