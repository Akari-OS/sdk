...
Module が Agent Runtime に渡す context（ACE 準拠）を組み立てるための API 仕様。
...
Context API（`@akari/sdk` の `context` 名前空間）は、Module 開発者が ACE 準拠の context を安全に組み立てるための SDK レイヤーである。直接 prompt 文字列を組み立てる代わりに、型付きの `ContextItem` を積み上げ、SDK が token budget 管理・優先度ソート・Lint チェックを行う。
...
type ContextItemKind =
  ...
  | "system-note"      // Module が追加するシステム的注記
```
...
type ContextItemSource =
  ...
  | { type: "inline" }   // Module が直接組み立てたインラインデータ
```
...
## 6. Handoff（Context Handoff）— Module 間 context 引き継ぎ
...
### 6.4 Module 間 Handoff（Inter-App）
...
// Video Module が Writer からの handoff を受け取る
module.onHandoff(async (payload) => {
...
Writer Module が「下書きを磨く」エージェントを呼び出す完全なフロー。UI Selection（テキスト範囲選択）と AMP の style memory を組み合わせる。
...
- **HUB-024** [`Module SDK spec (AKARI-HUB-024, Hub)`](https://github.com/Akari-OS/.github/blob/main/VISION.md) — Module SDK 全体（Context API は §5.3 / §6.6）
