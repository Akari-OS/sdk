...
AKARI Module SDK を使った開発でつまずいたとき、まずここを見る。
...
│   └── handoff が target 側で拒否される  → §4.4 Inter-App handoff 拒否
│
├── UI が表示されない / おかしい
...
または `akari-module-cli create` 実行時にシンタックスエラーが出る。
...
# nvm 推奨（sudo 不要）
nvm install 20
npm install -g akari-module-cli   # nvm 管理下では sudo 不要
...
source ~/.zshrc
npm install -g akari-module-cli
```
...
または `akari-module-cli` がインストール済みなのに `akari` コマンドが動かない。
...
```bash
# グローバルインストール確認
npm list -g akari-module-cli

# PATH 確認
echo $PATH | tr ':' '\n' | grep npm

# ローカル実行（npx）
npx akari-module-cli create my-module

# または package.json の scripts 経由
# "scripts": { "create": "akari-module-cli create" }
...
```
[LINT ERROR] AKR-G002: Module must not create its own database.
...
```
[LINT ERROR] AKR-G003: Module must not create windows outside Shell panel.
...
```
[LINT ERROR] AKR-G004: Direct inter-module communication is forbidden.
  Found: fetch('akari://module/com.akari.writer/api/...') at src/handoff.ts:34
```

**原因**

Guidelines §6.7-4「エージェント間通信は記憶層経由」違反。Module を直接叩いている。

**解決**

```typescript
// NG: 他 Module のエンドポイントを直接叩く
const res = await fetch('akari://module/com.akari.writer/api/draft')
...
```
[LINT ERROR] AKR-C001: Module category 'publishing' does not match declared tools.
...
または Shell に Module を追加しても Panel が空白のまま。
...
1. Shell の Module 設定 → 「Notion の認証を更新」ボタンをクリック
2. ブラウザで再度 Notion 認証ページを開き、workspace を選択
3. token が Keychain に保存され直されたことを確認

**再現防止**

`settings` panel に「認証の有効期限」を表示する機能を Module に実装しておくと、ユーザーが事前に気づける。
...
[RUNTIME ERROR] Permission denied: pool.write
  Module 'com.x.sender' does not have 'pool.write' permission.
...
### 4.4 Inter-App handoff が target 側で拒否される

**症状**

Shell に「○○ Module が handoff を受け取れませんでした」トーストが出る。

または：

```
[RUNTIME ERROR] Handoff rejected by 'com.akari.video'.
  Reason: intent 'create-video-from-draft' is not declared in target module's manifest.
```

**原因 A** — 受け取り側が `intent` を宣言していない

handoff の `intent` は受け取る側の Module が manifest で受け入れを宣言しておく必要がある。
...
**原因 C** — 受け取り側 Module がインストールされていない

```bash
# Module がインストールされているか確認
akari module list | grep com.akari.video

# インストール
akari module add com.akari.video
```
...
Shell のサイドバーに Module のアイコンが出るが、Panel をクリックしても何も表示されない。
...
| 原因 | 解決 |
|---|---|
| `shell.mountPanel` の `id` に重複がある | Module ID を含んだユニーク名にする（例: `my-module.main`） |
...
`pool-picker` / `amp-query` / `module-picker` / `agent-picker` 等の AKARI 固有 widget は
...
1. `locales/ja.json` が Module ルートに存在するか確認

```bash
ls my-module/locales/
# ja.json  en.json
```
...
3. `akari.toml` の `[module]` に `defaultLocale` が設定されているか確認
...
Shell に Module を追加したあと、Panel が表示されるまで 3 秒以上かかる。
...
Module を長時間使っていると Shell 全体がだんだん重くなる。DevTools の Memory タブでヒープが増え続けている。
...
### Q: 自分の Module 固有エージェントと 7 つの reference agent（Persistent Identity）の違いは何か？

**A:**

AKARI Core には 7 人の Persistent Identity エージェントが常駐している（Orchestrator / Analyst / Creator 等）。これらは Core が管理するプラットフォーム資産であり、Module 開発者は定義も変更もできない。

一方、Module 固有エージェント（`agents/*.md`）は：

- **その Module の用途に特化**したオン・デマンド（揮発）エージェント
- Module が削除されれば消える
...
Core の 7 Persistent Agent
  ├── Module A の固有エージェント（Module A のインストール中のみ存在）
  ├── Module B の固有エージェント
  └── ...
```

Module 固有エージェントから Core の 7 Agent に処理を委譲したい場合は、Memory API（Pool / AMP）を介してデータを渡す。直接呼び出す手段は提供されていない。
...
akari module certify --only contract
...
`akari module publish` を実行すると Marketplace チームへの Review リクエストが送られる。
Review の主な観点：
...
### Q: 内部 AKARI spec ID（AKARI-HUB-024 など）を Module のコードやドキュメントに使っていいか？
...
await amp.record({
  ...
  goal_ref: 'AKARI-HUB-026',  // ← 自分のモジュールが実装している spec を指す
})
```

---

### Q: TypeScript / Node.js 以外の言語・ランタイムで Module を書けるか？
...
ビジネスロジック（`src/`）は TypeScript / JavaScript が基本だが、Rust Module については `akari-sdk-rs`（Rust crate）が HUB-024 §6.10 Tasks Phase 3 で実装予定。
...
Module 開発者が作った Module を商用利用・販売する場合は：
...
3. Module が外部サービス（Notion / X 等）の API を利用する場合は、その API の利用規約も遵守が必要
...
- **GitHub Issues**: [akari-os/akari-os/issues](https://github.com/akari-os/akari-os/issues)
  - バグ報告: `bug` ラベルを付けて `akari module certify` の出力全文を貼る
...
- **GitHub Discussions**: [akari-os/akari-os/discussions](https://github.com/akari-os/akari-os/discussions)
  - カテゴリ `Q&A` を選ぶ
  - Module ID（`com.xxx.yyy`）と SDK バージョン（`@akari/sdk x.y.z`）を明記する
...
**`akari module certify` の出力（Lint / Contract の全文）**
```paste here```
