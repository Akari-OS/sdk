...
App ルートに `.akari-lint.toml` を置くことで、特定ルールを `warning` に降格または無効化できる。

```toml
# .akari-lint.toml
[rules]
LINT-051 = "warning"   # 直接 App 間通信検知を warning に降格
...
### LINT-001: app id — 逆ドメイン形式

**チェック内容**: `[app] id` が `<org>.<...>.<name>` の逆ドメイン形式であること。
...
### LINT-002: app tier — 有効な Tier 値

**チェック内容**: `[app] tier` が `"full"` または `"mcp-declarative"` であること。
...
**補足**: `tier` フィールドを省略した場合、既定は `"full"` として扱われ `warning` が出る（`[app]` セクション下）。
...
### LINT-003: app sdk — semver range

**チェック内容**: `[app] sdk` が有効な semver range 文字列であること。
...
### LINT-004: app version — semver 形式

**チェック内容**: `[app] version` が `X.Y.Z` の semver 形式であること。
...
### LINT-005: app name — 存在確認

**チェック内容**: `[app] name` フィールドが存在し、空でないこと。
...
### LINT-006: app author — 存在確認

**チェック内容**: `[app] author` フィールドが存在し、空でないこと。
...
### LINT-013: [agents] キーに app-short-id プレフィックス必須
...
```toml
# [app] id = "com.akari.writer" の場合

# NG
[agents]
editor = "agents/editor.md"           # app-short-id prefix なし
...
**チェック内容**: `[app] category` が省略されている場合に `warning` を出す。

**補足**: カテゴリ未設定の App は Marketplace で「未分類」に格納され、発見されにくくなる。
...
INFO: category "x-education" はカスタムカテゴリです。
      ...
      利用 App が増えたら HUB-005 改訂で Core カテゴリへの昇格を申請できます。
```
...
**チェック内容**: App ソースコード内に以下のようなパターンが検出された場合。
...
### LINT-051: 直接 App 間通信の禁止

**チェック内容**: `app.handoff()` を使わず、他 App の内部 API や endpoint を直接呼び出しているパターン。
...
// NG: 直接 HTTP で他 App を叩く
await fetch("http://localhost:3002/writer-api/drafts")

// OK: Inter-App API 経由
await app.handoff({ to: "com.akari.video", intent: "...", payload: {...} })
```
...
| LINT-001 | A | error | ✅ | ✅ | app id 逆ドメイン形式違反 |
...
| LINT-013 | B | error | ✅ | — | agents キー app-short-id prefix なし |
...
**LINT-013: agents キーに app-short-id プレフィックスを追加**

```toml
# app id = "com.example.my-tool" → app-short-id = "my_tool"
