...
> マーケット掲載（`akari module publish`）時のみ必要。自己配布（npm / git）は不要。
>
> **Full Module** は全チェック項目が対象。
> **MCP-Declarative Module** は Permission / OAuth スコープの妥当性のみ（大幅に軽量）。
...
**適用**: Full Module 必須 / MCP-Declarative スキップ（Schema レンダラが担保）
...
**補足 UX-01**: `akari module certify` の Contract Test で計測された起動時間が参考値として提示される。
...
| # | チェック項目 | 重大度 | 確認方法 |
|---|---|---|---|
| UX-12 | Module の名前・説明が Module の実際の機能と一致している | required | 読み合わせ |
| UX-13 | AKARI OS の他の Module や Shell と競合する機能名を使っていない | required | 名称確認 |
| UX-14 | アイコン（icon フィールド）が適切で、他 Module のアイコンと混同されない | recommended | 目視 |

---

## 3. チェックリスト: セキュリティ / Security

**適用**: Full Module 必須 / MCP-Declarative は OAuth・Permission 項目のみ必須
...
| SEC-08 | OAuth スコープが Module の機能で実際に必要なものだけに限定されている | required | 機能比較 | ✅ | ✅ |
...
| SEC-14 | Pool / AMP に保存するデータが Module の機能に必要なものに限定されている | required | コードレビュー | ✅ | ✅ |
| SEC-15 | パスワードや秘密鍵を Pool / AMP に平文で保存していない | required | コードレビュー | ✅ | ✅ |
| SEC-16 | Module がユーザーに成り代わって HITL なしに外部サービスへ投稿・送信していない | required | コードレビュー | ✅ | ✅ |
...
**計測方法**: `akari module certify --profile` でパフォーマンスプロファイルが出力される。
この値がレビュー時に参照される。
...
| # | チェック項目 | 基準 | 重大度 |
|---|---|---|---|
| PERF-10 | Module バンドル（JS）の合計サイズ | ≤ 5 MB（gzip 後） | required |
...
**適用**: Full Module 必須 / MCP-Declarative 任意（推奨）
...
**適用**: Full Module 必須 / MCP-Declarative は最低限必須

### 6.1 必須ドキュメント

| # | チェック項目 | 重大度 | Full | MCP-D |
|---|---|---|:-:|:-:|
| DOC-01 | `README.md` が存在し、Module の目的・機能を 1 パラグラフで説明している | required | ✅ | ✅ |
...
| # | チェック項目 | 重大度 | Full | MCP-D |
|---|---|---|:-:|:-:|
| DOC-08 | Module のビルド手順・開発環境のセットアップ手順がある | recommended | ✅ | opt |
...
| # | チェック項目 | 重大度 | Full | MCP-D |
|---|---|---|:-:|:-:|
| DOC-11 | `akari.toml` の `[module] name` / `description` が適切に記載されている | required | ✅ | ✅ |
...
### 7.1 Full Module と MCP-Declarative の審査差分

```
Full Module                      MCP-Declarative Module
───────────────────────────────  ──────────────────────────────────
...
1. **OAuth スコープの最小権限**（SEC-07 / SEC-08）
   ...
   - 例: 「投稿のみ」の Module が Read スコープまで要求していないか

2. **MCP ツールの宣言と実際の動作の一致**（DOC-03 相当）
   - `[mcp] tools` に宣言したツールが、Module の説明に書かれた機能と一致しているか

3. **external-network ドメインの妥当性**
   - `external-network = ["api.x.com"]` のように宣言されたドメインが、Module の機能に必要なものに限られているか

4. **README の Permission 説明**（DOC-03）
   - なぜその Permission / OAuth スコープが必要かが、ユーザーが理解できる言葉で書かれているか

### 7.3 Full Module の審査で特に厳しく見る項目
...
# パフォーマンスプロファイルも出力
akari module certify --profile

# Manual Review 用のセルフチェックリストを出力
akari module certify --checklist manual
```

`akari module certify --checklist manual` は本ページのチェックリストを Tier に合わせて絞り込んで出力する。
...
# 審査ステータス確認
akari module publish status

# 修正後の再申請
akari module publish --resubmit
```

### 8.3 提出時に同梱するもの

`akari module publish` を実行する前に、以下を準備する:

| ファイル | 必須度 | 内容 |
|---|---|---|
| `README.md` | 必須 | Module の説明・インストール手順・Permission 理由 |
...
1. `akari module publish appeal` を実行
2. 審査員の指摘に対する反論・説明を記述
3. AKARI チームが 5 営業日以内に再審査

異議申し立ては 1 Module あたり 1 回まで。再審査後の判定は最終とする。
