...
> Module をマーケットに掲載するための品質審査システム。
> Apple の App Store Review に相当する、AKARI 版の品質ゲート。
> `akari module certify` の 1 コマンドで 3 層すべてを実行できる。
...
AKARI Module の Certification は **3 層構造** になっている。

| 層 | 名称 | 実行主体 | Full | MCP-Declarative |
|---|---|---|:-:|:-:|
| 1 | **Automated Lint** | `akari module certify` が自動実行 | ✅ 必須 | ✅ 必須 |
| 2 | **Contract Test** | `akari module certify` が自動実行 | ✅ 必須 | ✅ 必須 |
...
マーケット掲載（`akari module publish`）は層 3 の Manual Review が追加される。
...
|---|:-:|:-:|---|
| `[module] id` 逆ドメイン形式 | ✅ | ✅ | `LINT-001` |
| `[module] tier` 宣言 | ✅ | ✅ | `LINT-002` |
| `[module] sdk` 互換範囲 | ✅ | ✅ | `LINT-003` |
| `[module] version` semver | ✅ | ✅ | `LINT-004` |
...
## 4. CLI 使い方 / How to Use `akari module certify`

### 4.1 基本コマンド

```bash
# カレントディレクトリの Module を全層チェック
akari module certify

# 特定ディレクトリを指定
akari module certify --path ./my-module

# 層を指定して実行（デバッグ用）
akari module certify --only lint
akari module certify --only contract
akari module certify --only lint,contract

# JSON レポート出力（CI 連携用）
akari module certify --output json > certify-report.json

# 失敗しても終了コード 0 で返す（CI でのベストエフォート確認）
akari module certify --no-fail-on-error

# verbose 出力（各ルールの詳細ログ）
akari module certify --verbose
```
...
───────────────────────────────────────
 ...
 [1/3] Automated Lint
  ✓  LINT-001  module id (com.example.my-module) — reverse-domain ✓
 ...
 [3/3] Manual Review
  ...
  ℹ  Run `akari module publish` to initiate Manual Review for marketplace listing
...
```
 [1/3] Automated Lint
  ✗  LINT-001  module id "my-module" — must be reverse-domain (e.g. com.org.name)
  ✓  LINT-002  tier = mcp-declarative
...
jobs:
  certify:
    ...
    steps:
      ...
      - run: akari module certify --output json | tee certify-report.json
...
はい。自己配布の場合、ユーザーは自己の責任でインストールします。`akari module certify` が層 1 + 層 2 をパスすれば `PASS` 判定になります。ただし、Shell は「マーケット未検証」のバッジを Module に付与します。

**Q: MCP-Declarative Module は Manual Review が「軽め」とありますが、何がスキップされますか？**
...
**Q: `akari module certify` が社内 CI からタイムアウトします。**
...
- **仕様書（正典）**:
  - [AKARI-HUB-024 Module SDK spec (AKARI-HUB-024, Hub)](https://github.com/Akari-OS/.github/blob/main/VISION.md) — Certification §6.8 / テスト戦略 §8
