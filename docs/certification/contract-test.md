...
> Certification 第 2 層。Module が AKARI Core の **7 API の契約**を正しく実装しているかを
> **動的に**検証する。Lint（静的解析）が「形式」を見るのに対し、Contract Test は「動作」を見る。
> `akari module certify` がモックランナーを起動し、すべてのスイートを自動実行する。
...
「Module が SDK の契約（trait / interface）を満たしているか」を確認するのが目的であり、
Module の内部ロジック（ビジネスロジック）をテストするものではない。

```
┌─────────────────────────────────────────────────────┐
│ Contract Test の役割                                 │
│                                                     │
│  Module ←→ Core SDK の境界（contract）をテストする   │
│                                                     │
│  Module 内部のロジック（Unit Test）はテストしない     │
│  → それは Module 開発者自身の責任                    │
└─────────────────────────────────────────────────────┘
```
...
`akari module certify` が起動するモックランナーは、AKARI Core の各 API を **インメモリモック**で提供する。Module はこのモックに対して実際に SDK を呼び出す。
...
// 検証対象
defineAgent({
  id: string,               // 必須: "<module-short-id>_<role>" 形式（ADR-011）
...
`defineAgent({ id })` が `<module-short-id>_<role>` の snake_case 形式であること。

**合格条件**: `id` が `^<module-short-id>_[a-z][a-z0-9_]*$` にマッチする。  
...
**不合格例**: `goal_ref` が省略されている場合、モックが `AMP_GOAL_REF_REQUIRED` エラーを返す → Module がそれを無視して処理を続けていたら FAIL。
...
### CT-2.7: Pool のデータは AMP 経由で Module 間共有

**シナリオ**: Module A が `pool.put()` し、Module B が `pool.get(id)` できること。
モックランナーが共有ストアを提供するため、ID を渡せば別 Module からも参照できる。

**合格条件**: Module が put した ID を別の「仮想 Module B コンテキスト」から get できる。
...
```typescript
shell.mountPanel({
  id: string,           // 必須: "<module-id>.<panel-name>"
  title: string,        // 必須
  component: Component, // Full: 必須 / MCP-Declarative: 不要
})
```

**合格条件**: 上記フィールドが存在する。`id` が `"<module-id>."` プレフィックスを含む。

---

### CT-4.2: mountPanel — id のプレフィックス

`mountPanel.id` が `<module-id>.` で始まること。

```typescript
// NG
shell.mountPanel({ id: "main", ... })

// OK (module id = "com.akari.writer")
...
`module.handoff()` の `payload` フィールドに Pool / AMP の **ID のみ** を渡し、bytes を直接含めないこと。
...
`to` フィールドが逆ドメイン形式の Module ID であること。
...
`module.handoff()` 呼び出し後、モックの AMP に監査ログが自動記録されること。
...
**合格条件**: Module が `HITL_REJECTED` 例外を適切にハンドリングする（クラッシュしない）。
...
`skill.register.id` が `<module-short-id>.` で始まること（ADR-011 準拠）。
...
**合格条件**: `id.startsWith(moduleShortId + ".")`
...
`[permissions] external-network = false` が宣言されている場合、ネットワークを完全にブロックした状態で Module の基本機能（Panel 描画 / Agent 呼び出し / Memory アクセス）が動作すること。
...
`external-network` が宣言されているが、ネットワークが利用できない場合（タイムアウト）に Module がクラッシュせずフォールバック動作すること。
...
# 特定のスイートのみ実行
akari module certify --only contract --suite memory,permission

# verbose 出力（各テストケースの詳細）
akari module certify --only contract --verbose

# タイムアウト設定（デフォルト 60 秒）
akari module certify --only contract --timeout 120
```
...
Module 固有のテストケースを追加したい場合は `tests/contract/custom/` 配下に置く。
...
describe("My Module — Custom Contract", () => {
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          ...
          cache: "npm"
      - run: npm ci
      - run: npm install -g @akari/module-cli
      - name: Run Lint
        run: akari module certify --only lint --output json | tee lint-report.json
  ...
  contract-test:
    ...
    steps:
      ...
      - name: Run Contract Test
        run: akari module certify --only contract --output json | tee contract-report.json
        timeout-minutes: 5
      - uses: actions/upload-artifact@v4
        if: always()
        with:
      ...
      - name: Post Summary Comment
        ...
        with:
          script: |
            ...
            const body = `## Module Certification Report
...
jobs:
  certify:
    ...
    steps:
      ...
      - run: akari module certify --output json > certify-result.json
