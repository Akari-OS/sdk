...
> **対象読者**: AKARI Module を初めて作る開発者、既存 MCP サーバーを AKARI に載せたい開発者
...
**MCP-Declarative Tier** は AKARI Module の 2 つの Tier（Full / MCP-Declarative）のうち、**参入コストを 1/10 に抑えた軽量 Tier** です。
...
Full Module と MCP-Declarative Module の関係は、iOS の **Full App と App Clips** の関係に相当します。

| | iOS | AKARI |
|---|---|---|
| 重量版 | Full App（任意の Swift UI） | Full Module（任意の React Panel） |
| 軽量版 | App Clips（制限付き、即起動） | MCP-Declarative Module（JSON 宣言のみ、即載せ） |
| 共通点 | 同じ SDK・同じ審査基準 | 同じ SDK・同じ Permission / Memory / Skill 契約 |
| 違い | UI 自由度とストア審査の重さ | UI 自由度と Manual Review の深度 |

App Clips が「ダウンロード不要でカフェのメニューを表示する」ように、MCP-Declarative Module は「自社 API を AKARI に載せるための最短経路」です。
...
Full Tier で Module を作るには React Panel + Agent 定義 + Skill 実装が必要です。しかし、SNS 投稿・翻訳・外部検索・Notion 書き込みなどの **「外部サービスへの橋渡し」** は、UIとして必要なのは大半が「テキスト入力 + ファイル添付 + ボタン」の組み合わせです。
...
`akari-module-cli` で雛形を生成します：

```bash
akari-module-cli create my-service --tier mcp-declarative
```
...
```bash
akari-module-cli create x-sender --tier mcp-declarative --category publishing
akari-module-cli create notion    --tier mcp-declarative --category documents
akari-module-cli create deepl     --tier mcp-declarative --category translation
```
...
すべての書き込みツールに `dry_run: boolean` パラメータを実装することを推奨します。`akari module certify` の Contract Test はデフォルトで `dry_run: true` で実行されます。
...
| **AKARI 固有** | `pool-picker` / `amp-query` / `module-picker` / `agent-picker` | 記憶層参照 |
...
Notion Module の Database View のような複数 tab 構成の例：
...
`notion-module-panel.schema.json` のように、ロケールファイルを別ファイルにせず Schema 内の `locales` キーに埋め込む方法も使えます：
...
小規模な Module では外部ファイルより管理しやすい場合があります。
...
コールバック受信 → access token + refresh token
     ↓
Keychain に保存（service: com.akari.<module-id>）
     ↓
...
MCP-Declarative Module は **「書き込み系だけネットワーク依存、読み書き先行は Pool キャッシュ」** の方針で設計します。
...
最もシンプルな MCP-Declarative Module の例です。ポイント：
...
4. **Writer からの handoff を受け取る**
   ```
   Writer → module.handoff({ to: "com.akari.x-sender", intent: "post-draft",
...
より複雑な MCP-Declarative Module の例です。ポイント：
...
4. **クロスオーバー（Writer / Research との連携）**
   ```
   Writer → Notion handoff（下書き → Notion ページ化）
   Research → Notion handoff（AMP 収集結果 → database 行追加）
...
実 Schema は [notion-module-panel.schema.json](../examples/notion/panel.schema.json) を参照。全文仕様は [spec-module-notion-reference.md](../examples/notion/) (HUB-026) を参照。
...
|---|---|---|
| 任意の React コンポーネント | 汎用レンダラのみ | Full Module の Panel（`.tsx`） |
| カスタムアニメーション | Shell Theme が管理 | Full Module |
| Canvas / WebGL 描画 | Shell レンダラ対象外 | Full Module |
| 独自フォント・カスタム配色 | Shell Theme 統一 | Full Module |

### ロジック面での制約

| 制約 | 理由 | 代替手段 |
|---|---|---|
| 自前 DB（SQLite / IndexedDB 等） | **Module は自前 DB を持たない**（HUB-024 Guidelines §6.7） | Pool / AMP 経由 |
| 他 Module への直接呼び出し | 記憶層経由が必須 | Inter-App API（Pool / AMP ID 渡し） |
| リアルタイム描画（60fps 更新） | Schema レンダラの更新サイクルに依存 | Full Module |
| Module 内部 state の永続化 | Module は state を持たない | Pool / AMP に書く |

### セキュリティ制約

- credentials（OAuth token / API key）は **必ず Keychain 経由**（平文保存禁止）
- `external-network` に宣言していない URL への通信は Core が遮断します
- HITL 必須アクションで `hitl.require: false` にすると `akari module certify` が reject します
...
- 他 Module に公開するスキル（`skills/` 配下）が必要
...
4. `akari module certify` を再実行（Manual Review が追加される）
...
`goal_ref` なしの AMP レコードは `akari module certify` で警告が出ます。
...
SNS 投稿・外部書き込み・削除系アクションは必ず `hitl.require: true` が必要です。省略すると `akari module certify` で reject されます。
...
### pitfall 5: Module 内に自前 DB を持つ

```typescript
// 悪い例
import Database from "better-sqlite3"
const db = new Database("my-module.db")  // 禁止！
...
自前 DB は `akari module certify` の Automated Lint で検出されます。
...
- **[Inter-App API](../../api/inter-app-api.md)**（今後公開予定）: `module.handoff`

### 仕様書

- **[HUB-024 Module SDK](https://github.com/Akari-OS/.github/blob/main/VISION.md)**: Tier 定義 / 7 API / Certification / Toolchain
- **[HUB-025 Panel Schema v0](https://github.com/Akari-OS/.github/blob/main/VISION.md)**: Widget Catalog / Binding / Action / Validation
- **[HUB-005 Declarative Capability Modules](https://github.com/Akari-OS/.github/blob/main/VISION.md)**: 11 カテゴリ / 共通パターン / Reference Implementation 一覧

### 参考実装

- **[HUB-007 X Sender（Publishing）](../examples/x-sender/)**: シンプルな投稿 Module の全仕様
...
- **[Module Certification Guide](../certification.md)**（今後公開予定）: `akari module certify` の実行方法と各検証項目の詳細
...
*このガイドは AKARI Module SDK spec（HUB-024 / HUB-025 / HUB-005 / HUB-026 / HUB-007）を元に作成しています。spec との差異がある場合は spec が正とします。*
