...
Full Tier Module の開発に必要なすべてを 1 ドキュメントに集約したリファレンス。
本ガイドは **AKARI Module SDK（HUB-024）** の Tier 固有の解説に集中する。
...
**Full Tier は AKARI Module SDK の「重量級」Tier**。任意の React コンポーネントで Panel を実装でき、Module 固有の Agent を同梱し、Skill を他 Module に公開できる。UI 自由度と引き換えに、Certification の審査（特にマーケット掲載時の Manual Review）が MCP-Declarative Tier より重くなる。
...
- **Module 固有 Agent** (`agents/*.md`) を Agent Runtime に登録し、on-demand で呼び出せる
- **Skill** を他 Module に公開できる
...
- Module 固有の専門 Agent（文章編集者 / カラーグレーディング AI 等）を持ちたい
- Skill を他 Module に公開して連携エコシステムを作りたい
...
Core の 7 Agent（Partner / Studio / Operator / Researcher / Guardian / Memory / Analyst）はデフォルトとして存在するが、特定ドメインに特化した Agent（例: 文章編集者、カラーグレーディング AI）は Module-supplied agent として `agents/` 配下で定義できる。

---

## 3. プロジェクト構造 / Project Structure

`akari-module-cli` で Full Tier の雛形を生成するコマンド：

```bash
akari-module-cli create my-module --tier full
```
...
```
my-module/
├── akari.toml                  ← Module Manifest（Tier 宣言含む）
│
├── agents/                     ← Module 固有 Agent 定義
...
│
├── tests/
│   └── contract/               ← Module Contract Test（Certification 必須）
│       └── api.test.ts
│
├── package.json
├── tsconfig.json
...
> `src-tauri/` はネイティブ処理が不要なら省略できる。FFmpeg / GPU 等を使わない Full Tier Module（Writer 等）は `src-tauri/` を持たない場合が多い。
...
# ─── Module 固有 Agent（ADR-011 命名規約に従う） ──────────────────────────────
[agents]
# キー = エージェント ID。必ず "<module-short-id>_<role>" の snake_case にすること
# （module-short-id = akari.toml [module] id の末尾セグメントを snake_case 化したもの）
my_editor_writer   = "agents/writer.md"
my_editor_reviewer = "agents/reviewer.md"

# ─── 公開 Skill（他 Module が呼び出せる関数） ──────────────────────────────
[skills.exposed]
"my-editor.generate_draft" = "skills/generate_draft.ts"
"my-editor.extract_outline" = "skills/extract_outline.ts"

# ─── 利用する他 Module の Skill ────────────────────────────────────────────
[skills.imported]
"pool.search"       = ">=0.1"
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `[module].id` | 必須 | 逆ドメイン形式。マーケットでの一意識別子 |
| `[module].tier` | 推奨明示 | `"full"` を宣言。省略時も Full だが明示が可読性に貢献 |
| `[module].sdk` | 必須 | 互換 SDK 範囲。起動時チェックされ範囲外は実行拒否 |
| `[permissions]` | 必須 | 宣言した権限のみ実行時に取得可能。Lint で未宣言アクセスを検出 |
| `[panels]` | 必須（Full） | `mount` に React コンポーネントのパス指定 |
| `[agents]` | 任意 | Module 固有 Agent。ADR-011 の命名規約（`<short-id>_<role>`）必須 |
...
// Module の entry point で一度だけ呼ぶ
shell.mountPanel({
  id: "my-editor.main",       // グローバル一意。<module-id>.<panel-name> が慣習
  title: "My Editor",
...
Module 固有 Agent は `agents/` 配下に Markdown ファイルで定義する。
...
Module-supplied agent の ID は必ず **`<module-short-id>_<agent-role>`** の snake_case にする（ADR-011）。

```
module id (akari.toml): com.example.my-editor
                               ↓ 末尾セグメントを snake_case 化
module-short-id:        my_editor
...
Certification の Automated Lint は `<module-short-id>_` prefix を強制チェックする。
...
Module-supplied agent は Agent Runtime（LaunchAgent として常駐する daemon）に登録される。
Shell は Unix domain socket 経由で daemon と通信し（ADR-007 §11）、Module の `agents/` 定義は daemon がロードする。

```
Module の agents/writer.md
         │
         ▼（Module インストール時に自動登録）
...
Module をアンインストールすると `my_editor_writer` は自動で登録解除される。
...
Module 側で明示的に記録する必要はないが、goal_ref を含む記録を自分でも残すと可読性が上がる：
...
ネイティブ処理の結果は必ず Pool に格納する（Module が自前 DB を持つことは禁止）：
...
ADR-007 §9 で確定したプラグイン群。Module 側 `src-tauri/Cargo.toml` に必要なものを追加する：
...
**Module 本体に state を持たない。全て Memory Layer（Pool / AMP）に書く。** これは HUB-024 §6.7 の Guideline #9 に明記されている。

```typescript
// NG: Module 内に揮発するメモリ上の state を保持する
...
SDK の 7 API 群をすべて使う唯一の公式 Module として設計されている。
...
| Inter-App API | `module.handoff({ to: "com.akari.x-sender", ... })` で Publishing Module に AMP/Pool ID のみを渡す |
| Permission API | `permission.gate({ action: "external-network.post", hitl: true })` で投稿前に HITL 承認 |
| Skill API | `skill.register({ id: "writer.generate_draft", ... })` で他 Module への Skill 公開 |

### 11.2 Writer の manifest を読む際の注意点

Writer の `akari.toml` は `[bundled]` セクションに注目：
...
`[bundled]` に宣言したリソースは Module インストール時に自動登録される。
ユーザーが「Module を追加」ボタンを押した瞬間に、Pool スキーマ / Memory スキーマ / Agent が全部そろった状態になる（Writer spec §6.1「インストール時に起きること」参照）。
...
Module がローカルに SQLite を持つことは**ガイドライン違反**（HUB-024 §6.7 #2）。
Certification の Lint が `tauri-plugin-sql` を Module 固有の方法で使っていないかをチェックする。

```typescript
// NG: Module 内で直接 SQLite を使う
import Database from "@tauri-apps/plugin-sql"
const db = await Database.load("sqlite:my-module.db")  // ← 禁止
...
`module.handoff()` には **Pool / AMP の ID のみ**を渡す。bytes を直接 payload に入れてはいけない（HUB-024 §6.6 Inter-App API 原則）。
...
| **Module SDK spec (AKARI-HUB-024, Hub)** | HUB-024 — Module SDK の仕様正典（7 API / Tier 制度 / Certification） |
| **spec-akari-writer.md** | Writer — Full Tier の公式リファレンス実装 |
| **ADR-007** | Shell Tech Stack — Tauri + React + Zustand + TanStack Router の選定理由 |
| **ADR-011** | Module Agent Naming Convention — `<module-short-id>_<role>` の根拠 |

---

*本ガイドの最新版は `docs/guides/sdk/tiers/full-tier.md`（`akari-os` リポ）。*
*仕様の正典は Module SDK spec (AKARI-HUB-024, Hub)。*
